...
def main():
    ...
    st.write("Para cada cliente é preciso calcular cada uma das componentes abaixo:")
    st.write("- Recência (R): Quantidade de dias desde a última compra.")
    st.write("- Frequência (F): Quantidade total de compras no período.")
    st.write("- Valor (V): Total de dinheiro gasto nas compras do período.")
    st.write("E é isso que iremos fazer abaixo. ")
    st.markdown("---")
    
    image = Image.open("bank_icon.png")
